...
Esta es la ecuación que se itera dentro de la función que permite calcular ODEs con este método numérico.
...
La idea del método RK2 es utilizar el punto medio para evaluar el método de Euler, como se indica en la figura. Mientras que el método de Euler se aplica en el punto $t$ para evaluar la derivada que aproxima la función en el punto $x = t + h$, el método RK2 utiliza el punto medio $t + h/2$.
...
El término de orden $h^2$ desaparece y nuestra aproximación tiene un error de orden $h^3$. Recordemos que incrementar el orden del error por un orden de magnitud es muy beneficioso a nivel computacional. Esta es la función que se itera en el método númerico, omitiendo el error asociado.

Ahora, el único problema es que requerimos conocer el valor de la función en el punto medio $x(t + h/2)$, el cual desconocemos. Para aproximar este valor utilizamos el método de Euler con un paso $h/2$, $(x + h/2) = x(t) + \\frac{h}{2}f(x,t)$. De esta manera, obtenemos las ecuaciones del método RK2:
...
Estas ecuaciónes se iteran dentro de la función del método numérico para obtener sus valores y la solución a la ODE.
...
De la misma manera que anteriormente estas ecuaciónes se iteran dentro del método numérico para obtener la solución a la ODE.
